<?php 
	...
	$sql = 'insert denuncias(autor, materia) values("'.$userData['email'].'", '.$_POST['matId'].')';
	$rs = mysql_query($sql);
	...
	if($rs){
		...
		$juriSelec = '(select email from juriSelecionado)';
		$sql = 'select email from usuarios where email <> "'.$userData['email'].'" and email <> '.$autor.' and not email in '.
		$juriSelec.' limit 10';
		$rs = mysql_query($sql);
